...
- **Rooms (v2):** `rooms list` / `rooms show <room_id>` / `rooms delete <room_id>` operate on `/api/v2/rooms/*` for debugging. `rooms events <room_id>` (live SSE mitschnitt) and create/invite/rename/reactions/undo/cancel are Follow-ups.
